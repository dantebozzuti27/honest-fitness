...
export default function ActiveWorkout() {
  ...
  const parseTimeToSeconds = (raw: any): number | null => {
    ...
    if (!s) return null
    if (s.includes(':')) {
      const [mm, ss] = s.split(':').map((v: any) => String(v).trim())
      const m = Number(mm)
      const sec = Number(ss)
  ...
  const formatLastSummary = (exRow: any) => {
    ...
    const strength = sets.filter((s: any) => s?.weight || s?.reps)
    if (strength.length > 0) {
      // choose top set by weight (fallback to last)
      const toDisplayWeight = (s: any) => {
      ...
      const sortWeight = (s: any) => {
        const w = toDisplayWeight(s)
        return typeof w === 'number' && Number.isFinite(w) ? w : 0
      }
      const top = strength.slice().sort((a: any, b: any) => (sortWeight(b) - sortWeight(a)))[0] || strength[strength.length - 1]
    ...
    const withTime = sets.filter((s: any) => s?.time != null && String(s.time).trim() !== '')
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const wanted = new Set(exercises.map((e: any) => normalizeNameKey(e?.name)).filter(Boolean))
  ...
  const inferSessionTypeFromExercises = (exs: any) => {
    const list = Array.isArray(exs) ? exs : []
        ...
        if (!hasResumedPaused && mounted) {
          if (templateId) {
            ...
            if (template && Array.isArray(template.exercises)) {
              const workoutExercises = template.exercises.map((entry: any, idx: number) => {
    ...
    async function loadWorkoutSession() {
      ...
      try {
        ...
        if (session) {
          ...
          workoutStartTimeRef.current = ws
          const elapsed = Math.floor((Date.now() - ws - (session.paused_time_ms || 0)) / 1000)
  ...
  const updateSet = (exerciseId: any, setIndex: number, field: any, value: any) => {
    setExercises(prev => prev.map(ex => {
  ...
  const removeSet = (exerciseId: any) => {
    setExercises(prev => prev.map(ex => {
  ...
  const finishWorkout = async (
    { navigateTo = null }: { navigateTo?: string | null } = {}
  ) => {
  ...
  const toggleExerciseStack = (exerciseId: any) => {
    setExercises(prev => {
  ...
  const addToStack = (exerciseId: any, targetStackGroup: any) => {
    setExercises(prev => {
  ...
  const removeFromStack = (exerciseId: any) => {
    setExercises(prev => prev.map(ex => {
      if (ex.id === exerciseId) {
        return { ...ex, stacked: false, stackGroup: null }
      }
  ...
  const unstackGroup = (groupId: any) => {
    if (!groupId) return
    setExercises(prev => prev.map(ex => {
  ...
  const handleStackNext = ({ exerciseId, stackGroup, isLastSet }: { exerciseId: any; stackGroup: any; isLastSet: any }) => {
  ...
  const filteredRenderItems = exerciseFilterKey
    ? renderItems
        .map((item) => {
          if (item?.kind === 'stack') {
            const members = (Array.isArray(item.members) ? item.members : []).filter((m: any) =>
    ...
    <SafeAreaScaffold>
      ...
      <div className={styles.content}>
        ...
        {filteredRenderItems.map((item) => {
              ...
              <div key={`stack-${item.groupId}`} className={styles.stackGroupBox}>
                <div className={styles.stackGroupHeader}>
                  <div className={styles.stackGroupTitle}>
                    {label}: {members.map((m: any) => m.name).join(' / ')}
                  </div>
                  <Button
                    unstyled
                  ...
                  {members.map((exercise: any) => {
                    ...
                    return (
                      <ExerciseCard
                        ...
                        onUpdateSet={(setIdx: number, field: any, value: any) => updateSet(exercise.id, setIdx, field, value)}
          ...
          const stackGroup = exercise.stacked ? exercise.stackGroup : null
          const stackMembers = stackGroup ? exercises.filter((ex: any) => ex.stacked && ex.stackGroup === stackGroup) : []
          const stackIndex = stackMembers.findIndex((ex: any) => ex.id === exercise.id)
          ...
          return (
            <ExerciseCard
              ...
              onStackNext={handleStackNext}
              onToggleStack={() => toggleExerciseStack(exercise.id)}
              onAddToStack={(targetGroup: any) => addToStack(exercise.id, targetGroup)}
              onRemoveFromStack={() => removeFromStack(exercise.id)}
              isDragging={draggedId === exercise.id}
              onDragStart={(e: any) => handleDragStart(e, exercise.id)}
              onDragOver={(e: any) => handleDragOver(e, exercise.id)}
              onDragEnter={handleDragEnter}
              onDragEnd={handleDragEnd}
              onDrop={(e: any) => handleDrop(e, exercise.id)}
            />
          )
        })}

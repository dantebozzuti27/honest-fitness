...
BEGIN
  ...
  CASE goal_record.type
    ...
    WHEN 'calories', 'calorie_intake' THEN
      ...
      SELECT COALESCE(calories_consumed, 0) INTO current_value
      FROM health_metrics
      WHERE user_id = goal_record.user_id
    ...
    WHEN 'calories_burned' THEN
      ...
      SELECT COALESCE(calories_burned, 0) INTO current_value
      FROM health_metrics
      WHERE user_id = goal_record.user_id
        AND date = CURRENT_DATE;
        
    WHEN 'protein', 'carbs', 'fat' THEN
      SELECT COALESCE((macros->>goal_record.type)::NUMERIC, 0) INTO current_value
      FROM health_metrics
      WHERE user_id = goal_record.user_id
        AND date = CURRENT_DATE;
    ...
    WHEN 'workouts_per_week' THEN
      ...
      SELECT COALESCE(COUNT(*)::NUMERIC, 0) INTO current_value
      FROM workouts
      WHERE user_id = goal_record.user_id
    ...
    WHEN 'steps' THEN
      SELECT COALESCE(steps, 0) INTO current_value
      FROM health_metrics
      WHERE user_id = goal_record.user_id
        AND date = CURRENT_DATE;
    ...
    WHEN 'sleep_hours', 'sleep' THEN
      ...
      SELECT COALESCE((sleep_duration / 60.0), 0) INTO current_value
      FROM health_metrics
      WHERE user_id = goal_record.user_id
        AND date = CURRENT_DATE;
        
    WHEN 'hrv' THEN
      SELECT COALESCE(hrv, 0) INTO current_value
      FROM health_metrics
      WHERE user_id = goal_record.user_id
        AND date = CURRENT_DATE;
  ...
  -- Ensure current_value is not NULL

export function formatSteps(steps: unknown): string | null {
...
export function formatSleep(raw: unknown): string | null {
  const n = Number(raw)
  if (!Number.isFinite(n) || n <= 0) return null
...
export function formatWeightLbs(weight: unknown): string | null {

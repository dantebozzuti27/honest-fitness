...
type BarChartPointData = Record<string, any>

type BarChartData = Record<string, number | string | null | undefined>
...
type BarChartProps = {
  ...
  yAxisLabel?: string
  dates?: string[] | null
  onBarClick?: ((payload: any) => void) | null
  dateData?: BarChartPointData | null
  chartTitle?: string
}
...
export default function BarChart({
  ...
  const [dataRangeEnd, setDataRangeEnd] = useState<number | null>(null) // For scale-based zoom
  ...
  const chartData = useMemo<{
    ...
    const keys = (labels || Object.keys(data)).map((k) => String(k))
  ...
  const handleBarClick = (index: number, value: number, key: string, date: string) => {
    if (onBarClick) {
  ...
  const visibleData = useMemo<VisibleData | null>(() => {

...
export type UnifiedChartDateRange = {
  start: string
  end: string
  indices: { start: number; end: number | null }
}

export type UnifiedChartProps = {
  ...
  height?: number
  color?: string
  onDateRangeChange?: (range: UnifiedChartDateRange) => void
  showValues?: boolean
  xAxisLabel?: string
  yAxisLabel?: string
}
...
export default function UnifiedChart({
  ...
  const handleTouchMove = (e: TouchEvent) => {
    if (e.touches.length === 2 && isPinching && touchStartRef.current) {
      ...
      if (onDateRangeChange && chartData) {
        ...
        onDateRangeChange({ start: String(startDate), end: String(endDate), indices: newRange })
      }
  ...
  const resetDateRange = () => {
    ...
    if (onDateRangeChange && chartData) {
      onDateRangeChange({ 
        start: String(chartData.keys[0]),
        end: String(chartData.keys[chartData.keys.length - 1]),

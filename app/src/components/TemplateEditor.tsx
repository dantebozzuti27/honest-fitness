...
import type { TemplateExercisePresetV2 } from '../utils/templateUtils'
...
type TemplateDraft = {
  id: string
  name: string
  exercises: TemplateExercisePresetV2[]
}

type DeleteConfirmState = { open: boolean; id: string | null; name: string }

type TemplateEditorProps = {
  templates: TemplateRow[]
  onClose: () => void
  onSave: (t: TemplateDraft) => void
  onDelete: (templateId: string) => void
  onEdit?: (t: TemplateRow | null) => void
...
export default function TemplateEditor({
  ...
  onDelete,
  onEdit,
  editingTemplate: initialEditingTemplate
}: TemplateEditorProps) {
  const [editingTemplate, setEditingTemplate] = useState<TemplateRow | TemplateDraft | null>(initialEditingTemplate || null)
  const [allExercises, setAllExercises] = useState<any[]>([])
  const [showPicker, setShowPicker] = useState(false)
  const { toast, showToast, hideToast } = useToast()
  const [deleteConfirm, setDeleteConfirm] = useState<DeleteConfirmState>({ open: false, id: null, name: '' })
  ...
  useEffect(() => {
    if (editingTemplate) {
      setFormData({
        ...
        exercises: normalizeTemplateExercises((editingTemplate as any).exercises)
      })
    }
  }, [editingTemplate])
  ...
  const handleAddExercise = (exercise: any) => {
    ...
    setFormData(prev => ({
      ...
      exercises: [
        ...prev.exercises,
        {
          name: String(exercise?.name || '').trim() || 'Exercise',
          sets: defaultSets,
          reps: isCardio ? '' : '8-12',
          time: isCardio ? '20:00' : '',
          notes: '',
  ...
  const patchExercise = (index: number, patch: Partial<TemplateExercisePresetV2>) => {
    setFormData(prev => ({
      ...prev,
      exercises: (prev.exercises || []).map((ex, i) => (i === index ? { ...(ex || { name: '', sets: '', reps: '', time: '', notes: '', stackGroup: null }), ...patch } : ex))
    }))
  }

  const getStackSize = (stackGroup: string | null) => {
    if (!stackGroup) return 0
    return formData.exercises.filter(e => e?.stackGroup && e.stackGroup === stackGroup).length
  }

  const stackWithNext = (index: number) => {
  ...
  const unstackExercise = (index: number) => {
  ...
  const handleNewTemplate = () => {
    ...
    const draft: TemplateDraft = { id, name: '', exercises: [] }

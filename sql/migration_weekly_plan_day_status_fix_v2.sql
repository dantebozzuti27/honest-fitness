-- Coerce completed-day workout id from JSON; strip stale actuals on non-completed days.
...
BEGIN
  IF NEW.day_status = 'completed' AND NEW.actual_workout_id IS NULL THEN
    IF NEW.actual_workout IS NOT NULL
       AND (NEW.actual_workout->>'id') ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$' THEN
      NEW.actual_workout_id := (NEW.actual_workout->>'id')::uuid;
    END IF;
  END IF;

  IF NEW.day_status IN ('planned', 'adapted', 'skipped') THEN
    NEW.actual_workout_id := NULL;
    NEW.actual_workout := NULL;
  END IF;

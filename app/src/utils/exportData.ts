...
type WorkoutExerciseLike = {
  ...
  category?: string
  body_part?: string
  workout_sets?: WorkoutSetLike[]
}

type WorkoutLike = {
  date?: string
  duration?: number
  workout_exercises?: WorkoutExerciseLike[]
}
...
type MetricLike = {
  ...
  sleep_time?: number | string
  hrv?: number | string
  steps?: number | string
  calories?: number | string
}

export async function exportWorkoutData(userId: string, userEmail: string): Promise<{ workouts: number; metrics: number }> {
  // Fetch all data
  const [workoutsRaw, metricsRaw] = await Promise.all([getWorkoutsFromSupabase(userId), getAllMetricsFromSupabase(userId)])
  const workouts = (Array.isArray(workoutsRaw) ? workoutsRaw : []) as WorkoutLike[]
  const metrics = (Array.isArray(metricsRaw) ? metricsRaw : []) as MetricLike[]
  ...
  const workoutSummary = workouts.map((w) => ({
    Date: w.date || '',
    Duration: `${Math.floor((w.duration ?? 0) / 60)}:${String((w.duration ?? 0) % 60).padStart(2, '0')}`,
  ...
  const exerciseDetails: Array<Record<string, unknown>> = []
  workouts.forEach((w) => {
